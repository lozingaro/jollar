...
define caricamentoSaldo{
  ...
  if(risposta == true){
    ...
    readFile@File(f)(res);
    synchronized( syncSaldo ){
      global.jollarIn = res.arraySaldi[1] //1 perchè il saldo del client 2 è nella seconda posizione dell'array!
    }
...
define scriviBlocco{
  ...
  synchronized( syncBlockchain ){
    ...
    block << ack.block;
    WriteFileRequest.filename = "FileClient2.json";
