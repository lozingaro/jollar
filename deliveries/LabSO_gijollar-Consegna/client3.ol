...
define caricamentoSaldo{
  ...
  if(risposta == true){
    ...
    readFile@File(f)(res);
    synchronized( syncSaldo ){
      global.jollarIn = res.arraySaldi[2] //2 perchè il saldo del client 3 è nella terza posizione dell'array!
    }
...
define scriviBlocco{
  ...
  synchronized( syncBlockchain ){
    ...
    block << ack.block;
    WriteFileRequest.filename = "FileClient3.json";
