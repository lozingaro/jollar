...
constants{
  LOCATION_CLIENT2 = "socket://localhost:8002/",
...
//creazione delle catene di numeri primi
...
//Caricamento del saldo dal file cash
define caricamentoSaldo{
  ...
  if(risposta == true){
    ...
    readFile@File(f)(res);
    synchronized( syncSaldo ){
      global.jollarIn = res.arraySaldi[0] //0 perchè il saldo del client 1 è nella prima posizione dell'array!
    }
    //println@Console( "preso il saldo dal file "+jollarIn )()
  }else{
    //altrimenti carico saldo iniziale di 6
    synchronized( syncSaldo ){
      global.jollarIn = 6
    }
  }
...
define caricamentoBlockchain{
  ...
  if(risposta == true){ //se esiste carico la blockchain
    ...
    caricamentoSaldo
  }else{//altrimenti il client 1 deve generare il primo blocco ed ottenere 6 jollar in reward
    genesys.transaction.nodeSellerID.publicKey = "Blocco genesi";
    genesys.transaction.nodeBuyerID.publicKey = "Blocco genesi";
    genesys.transaction.jollar = 0;
    genesys.transaction.time = 0;
    genesys.transaction.id = "00000";
    genesys.catena.numeriPrimi[0] = 0;
    genesys.catena.lunghezza = 1;
    requestTime@timestamp(request)(tempo);
    genesys.time = tempo;
    genesys.clientS.publicKey = MyPublicKey;
    genesys.id = string(new);
    genesys.previousId = " ";
    genesys.difficulty = 0;
    synchronized( syncSaldo ){
      global.jollarIn = 6
    };
    install( IOException => {
      println@Console("Errore nella scrittura del primo blocco della blockchain")()
    });

    WriteFileRequest.filename = "FileClient1.json";
    ...
    synchronized( syncBlockchain ){
      ...
      global.Blockchain.block[posizione]<<genesys;
      testo<<global.Blockchain
    };
    //stampo la blockchain
...
//serve a scrivere il blocco che ho creato nella mia BlockChain
define scriviBlocco{
  //cerco la posizione in cui aggiungere il blocco
  synchronized( syncBlockchain ){
    //println@Console( "entro in Scrivi blocco" )();
    block << ack.block;
    WriteFileRequest.filename = "FileClient1.json";
    posizione = #global.Blockchain.block;
...
//ricevute tutte le blockchain dagli altri client seleziono la più lunga
//a parità di lunghezza di due o più BlockChain seleziono quella presente in più client
...
init{
  ...
  clientCode = string(new); //key del client
  a.session.clientID = "client1";
  ...
  users[0] = LOCATION_CLIENT2;
  users[1] = LOCATION_CLIENT3;
  users[2] = LOCATION_CLIENT4;
...
main{
  [in(scelta)]{
    install(illegalNode => {
      println@Console( "Errore: numero del nodo" )();
      mostraOpzioni
    });
      ...
      if(numeroNodo == 2){
        ...
        scope (scopeName)
        {
          if(global.pkClient4 == " "){
            ...
            nodeBuyer.publicKey = global.pkClient4;
            nodeSeller.publicKey = MyPublicKey;
            tr.nodeBuyerID.publicKey = nodeBuyer.publicKey;
            tr.nodeSellerID.publicKey = nodeSeller.publicKey;
            tr.jollar = global.jollarOut;
...
[validationTr(request)(response){
  ...
  //METODO PER LA RICEZIONE DELLA CHIAVE DAL BROADCAST DI UN ALTRO CLIENT
  [sendKey (autentication)]{
    if( autentication.clientID == "client2" ) {
      ...
      //println@Console("Public Key client4: " + global.pkClient4)();
  ...
  //metodo che riceve la risposta della validazione del blocco creato
  [ackSendBlock( ack )]{
    if( ack.isOk == "true"){
      install( IOException => {
...
//metodo invocato dal network visualizer per ottenere i dati del client
  [requestData(request)(response) {
    //1 chiave del client
    response.pkClient = MyPublicKey;
    //2 saldo dei jollar
    response.saldo = global.jollarIn;
    //3 transazioni effettuate divise per entrate e uscite
  ...
  //METODO RICHIAMATO DAL BROADCAST PER OTTENERE TUTTE LE BLOCKCHAIN DEI NODI
  //ED EFFETTUARE COSì LA SINCRONIZZAZIONE
  [getBlockchain()(resp){
