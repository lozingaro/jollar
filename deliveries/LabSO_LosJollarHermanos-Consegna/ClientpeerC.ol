...
{
	sleep@Time(19000)();
	sendString@Netview("Nodo C inizia il mining");
	println@Console("Location: "+ClientC.location )() ;
	LocationClientC=ClientC.location;
	undef( req );
	...
	for(i=1,i<=resLoc,i++){
		if(LocationClientC!=resLoc.("figlio"+i)){
		...
		for(i=res3,i<=res3,i++){
			...
			if(m==1 && p[i]>res2.primo){ 
				for(j=1,j<=resLoc,j++){
					...
					ClientC.location=location2;
					controllo@ClientC(p[i])(confp); 
					confp2=confp2+confp;
					if(confp2==resLoc){
						println@Console("////////////////////////////////////////////////////////////")();
						wal=wal+6;
						...
						req.format="json";
						writeFile@File(req2)();
						sendString@Netview("NODO C"+"\n"+"Numero primo trovato "+ p[i] +"\n"+"CONFERMATO"+ "\n" + "Lunghezza catena: " +res2 + "\n"+ "C scrive il blocco, reward 6 jollar. Totale Wallet: " + wal+ "\n"+ "BLOCKCHAIN aggiornata")
					}
					else if (j==resLoc){
						println@Console("////////////////////////////////////////////////////////////")();
				...
				sleep@Time(15000)();
				ClientC.location= LocationClientC	
			}
			else if(m!=1){
				println@Console("////////////////////////////////////////////////////////////")();
				println@Console("Numero proposto: "+p[i])();
