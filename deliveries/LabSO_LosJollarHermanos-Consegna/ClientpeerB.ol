...
{
	sleep@Time(15000)();
	sendString@Netview("Nodo B inizia il mining");
	println@Console("Location: "+ClientB.location )() ;
	LocationClientB=ClientB.location;
	undef( req );
	req.filename="LocationPorte";
	req.format="json";
	readFile@File(req)(resLoc);
	for(i=1,i<=resLoc,i++){
		if(LocationClientB!=resLoc.("figlio"+i)){
		...
		for(i=res3,i<=res3,i++){
			...
			if(m==1 && p[i]>res2.primo){ 
				for(j=1,j<=resLoc,j++){
					...
					ClientB.location=location2;
					controllo@ClientB(p[i])(confp); 
					confp2=confp2+confp;
					if(confp2==resLoc){
						println@Console("////////////////////////////////////////////////////////////")();
						wal=wal+6;
						...
						writeFile@File(req2)();
						sendString@Netview("NODO B"+"\n"+"Numero primo trovato "+ p[i] + "\n" +"CONFERMATO" +"\n"+ "Lunghezza catena: " +res2 +"\n" +"B scrive il blocco, reward 6 jollar. Totale Wallet: " + wal+  "\n"+ "BLOCKCHAIN aggiornata")
					}
					else if (j==resLoc){
						println@Console("////////////////////////////////////////////////////////////")();
						println@Console("Numero proposto: "+p[i])();
				...
				sleep@Time(15000)();
				ClientB.location= LocationClientB	
			}
			else if(m!=1){
				println@Console("////////////////////////////////////////////////////////////")();
				println@Console("Numero proposto: "+p[i])();
